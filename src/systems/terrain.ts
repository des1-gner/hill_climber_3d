...
export type Biome = 'grassland' | 'forest' | 'rocky' | 'snow';
...
export function biomeAt(x: number, z: number): Biome {
  ...
  if (t < 0.3) return 'grassland';
  if (t < 0.55) return 'forest';
  if (t < 0.8) return 'rocky';
  return 'snow';
}

/** Driving friction per biome (within [0.05, 1.50], Req 6.5). Snow is icy. */
