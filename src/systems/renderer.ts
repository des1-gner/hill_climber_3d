...
const CHASE_OFFSET = new THREE.Vector3(0, 4, -9);
...
export class Renderer {
  ...
  renderFrame(interp: InterpolatedState): void {
    ...
    // Per-wheel ground speed derived from the interpolated forward speed.
    const groundSpeed = lerp(prev.linearSpeed, curr.linearSpeed, a);
  ...
  private updateChaseCamera(dt: number): void {
    ...
    this._camOffset.copy(CHASE_OFFSET).applyAxisAngle(AXIS_Y, yaw);
