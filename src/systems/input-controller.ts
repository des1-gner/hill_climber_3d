...
export class InputController {
   ...
   * frame (Req 5.5). Throttle/brake are normalized magnitudes (Req 5.1, 5.2)
   * and steering is expressed in degrees within the control bound (Req 5.3).
   *
   * Keyboard and touch contributions are combined; `resolveInput` later clamps
   * and arbitrates the result (Req 5.6, 5.7).
   */
  sample(): RawInput {
    // ArrowUp / W => forward throttle; ArrowDown / S => brake (Req 5.1, 5.2).
    ...
    // ArrowLeft / A => steer left (negative); ArrowRight / D => steer right
    // (positive). Held keys steer to the full control limit (Req 5.3).
    const left = this.pressed.has('ArrowLeft') || this.pressed.has('KeyA');
    const right = this.pressed.has('ArrowRight') || this.pressed.has('KeyD');

    let steer = 0;
    if (left) steer -= CONTROL_STEER_LIMIT;
    if (right) steer += CONTROL_STEER_LIMIT;

    // Combine with touch input. Keyboard takes precedence when both present,
    // but touch fills in any channel the keyboard leaves idle.
