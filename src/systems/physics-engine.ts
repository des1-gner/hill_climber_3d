...
export interface PhysicsEngine {
  ...
  applyCommand(cmd: DriveCommand, fuelEmpty: boolean): void;
  step(dt: number): void;
...
export class RapierPhysicsEngine implements PhysicsEngine {
  ...
  applyCommand(cmd: DriveCommand, fuelEmpty: boolean): void {
    ...
    // vehicle right. The renderer reads the applied wheel steering back for the
    // visual, so visuals stay consistent with this sign.
    const steerRad = -cmd.steerDeg * DEG_TO_RAD;
    ...
    if (cmd.throttle > 0) {
      ...
      engineForce = fuelEmpty ? 0 : cmd.throttle * this.config.maxEngineForce;
    } else if (cmd.brake > 0) {
      if (forwardSpeed > REVERSE_SPEED_THRESHOLD) {
        ...
        engineForce = fuelEmpty
          ? 0
          : -cmd.brake * this.config.maxEngineForce * REVERSE_FORCE_SCALE;
      }
    }
