...
function buildChassis() {
  ...
  for (const sx of [-1.04, 1.04]) {
    addPart(chassis, new THREE.BoxGeometry(0.18, 0.05, 0.05, 4, 2, 2), M.trim, [sx, 0.72, 1.06]);
    addPart(chassis, new THREE.BoxGeometry(0.06, 0.2, 0.24, 2, 4, 4), M.chrome, [sx + 0.12 * Math.sign(sx), 0.74, 1.06]);
  }
  ...
  // --- Wheel arches (torus arcs around each wheel, axle-aligned) ---
  const archGeo = new THREE.TorusGeometry(0.66, 0.1, 10, 24);
  for (const [ax, az] of [[-0.95, 1.5], [0.95, 1.5], [-0.95, -1.5], [0.95, -1.5]]) {
    addPart(chassis, archGeo, M.plastic, [ax, -0.18, az], [0, Math.PI / 2, 0]);
  }
